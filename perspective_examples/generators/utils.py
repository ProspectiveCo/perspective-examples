...
class RandomWaveGenerator:
    ...
    @staticmethod
    def _equals_parts(num_points: int = 1000, periods: int = 2) -> list[int]:
        """
    ...
    @staticmethod
    def _random_parts(num_points: int = 1000, periods: int = 2) -> list[int]:
        """
    ...
    @staticmethod
    def _half_waves_fixed_freq_varying_amp(
        num_points: int = 1000,                             # Number of data points
        ...
        **kwargs: Additional keyword arguments for wave modifications.
        ...
        """
        parts_length = RandomWaveGenerator._equals_parts(num_points, periods)
        parts = []
        for i in range(periods):
            sign = (-1) ** i
            y = sign * RandomWaveGenerator._half_wave(parts_length[i], amplitude=random.uniform(amplitude[0], amplitude[1]))
            parts.append(y)
        y = pd.concat(parts)
        # apply wave modifications
        if kwargs:
            y = RandomWaveGenerator._apply_wave_modifications(y, **kwargs)
        return y

    @staticmethod
    def _half_waves_varying_freq_varying_amp(
        num_points: int = 1000,                             # Number of data points
        periods: int = 3,                                   # Number of parts in the wave
        amplitude: tuple[float, float] = (1.0, 10.0),       # Amplitude of the wave (default is 1.0)
        **kwargs
        ...
        **kwargs: Additional keyword arguments for wave modifications.
        ...
        pd.Series: A pandas Series containing the generated wave data.
        """
        parts_length = RandomWaveGenerator._random_parts(num_points, periods)
        parts = []
        for i in range(periods):
            sign = (-1) ** i
            y = sign * RandomWaveGenerator._half_wave(parts_length[i], amplitude=random.uniform(amplitude[0], amplitude[1]))
    ...
    @staticmethod
    def _full_waves_fixed_freq_varying_amp(
        num_points: int = 1000,                             # Number of data points
        periods: int = 3,                                   # Number of parts in the wave
        amplitude: tuple[float, float] = (1.0, 10.0),       # Amplitude of the wave (default is 1.0)
        **kwargs
        ...
        **kwargs: Additional keyword arguments for wave modifications.
        ...
        parts = []
        for i in range(periods):
            y = RandomWaveGenerator._full_wave(parts_length[i], periods=1, amplitude=random.uniform(amplitude[0], amplitude[1]))
            parts.append(y)
        y = pd.concat(parts)
        # apply wave modifications
        if kwargs:
    ...
    @staticmethod
    def _full_waves_varying_freq_varying_amp(
        num_points: int = 1000,                             # Number of data points
        periods: int = 3,                                   # Number of parts in the wave
        amplitude: tuple[float, float] = (1.0, 10.0),       # Amplitude of the wave (default is 1.0)
        **kwargs
    ...
    @staticmethod
    def _apply_phase(wave: pd.Series, phase: float = 0.0) -> pd.Series:
        """
    ...
    @staticmethod
    def _apply_smooth(wave: pd.Series, window: int = 10) -> pd.Series:
        """
    ...
    @staticmethod
    def _apply_noise(wave: pd.Series, noise: float = 0.1) -> pd.Series:
        """
    ...
    def _apply_wave_modifications(
        ...
        """
        if phase:
            wave = RandomWaveGenerator._apply_phase(wave, phase)
        if smooth:
            wave = RandomWaveGenerator._apply_smooth(wave, smooth)
        if noise:
            wave = RandomWaveGenerator._apply_noise(wave, noise)
        return wave
    ...
    def sinusoidal_wave(
        ...
        function_map = {
            "full": {
                "fixed": RandomWaveGenerator._full_waves_fixed_freq_varying_amp,
                "amp": RandomWaveGenerator._full_waves_fixed_freq_varying_amp,
                "freq": RandomWaveGenerator._full_waves_varying_freq_varying_amp,
                "both": RandomWaveGenerator._full_waves_varying_freq_varying_amp,
            },
            "half": {
                "fixed": RandomWaveGenerator._half_waves_fixed_freq_varying_amp,
                "amp": RandomWaveGenerator._half_waves_fixed_freq_varying_amp,
                "freq": RandomWaveGenerator._half_waves_varying_freq_varying_amp,
                "both": RandomWaveGenerator._half_waves_varying_freq_varying_amp,
            },
        }
