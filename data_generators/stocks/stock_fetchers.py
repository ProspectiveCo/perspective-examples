...
class AlphaVantageStockFetcher:
    ...
    def __init__(
        ...
        if isinstance(intraday_interval, str):
            ...
            logger.info("Continuing with default intraday interval: 15 minutes.")
            intraday_interval = AlphaVantageIntradayInterval.FIFTEEN_MIN
        # setting the ticker_interval and intraday_interval
    ...
    def fetch_stock_values(
        ...
        params = {
            'function': self.ticker_interval.value,
            'symbol': ticker,
            'apikey': api_key,
        ...
        if self.ticker_interval == AlphaVantageTickerInterval.INTRADAY:
            params['interval'] = self.intraday_interval.value
        logger.info(f"Fetching Alpha Vantage Stock Values: ticker={ticker} start_date={start_date} interval={self.ticker_interval}")
    ...
    def _get_past_date_from_options(
        ...
        if nrows:
            ...
            elif self.ticker_interval == AlphaVantageTickerInterval.DAILY:
                relative_nrows_date = datetime.now() - timedelta(days=nrows)
            elif self.ticker_interval == AlphaVantageTickerInterval.WEEKLY:
                relative_nrows_date = datetime.now() - timedelta(weeks=nrows)
            elif self.ticker_interval == AlphaVantageTickerInterval.MONTHLY:
...
if __name__ == '__main__':
    fetcher = AlphaVantageStockFetcher(ticker_interval='INTRADAY', intraday_interval='15min')
    df = fetcher.fetch_stock_values('AAPL', time_span='-3d')
    print(df)
